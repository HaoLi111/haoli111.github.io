...
This is also where the work connects to the other threads in my research — agentic systems where the transition is much richer than a single forward pass, interpretability that asks *what* the transition is computing, AI for biology at [Paradigm.Study](https://www.paradigm.study/) and with [FuhaiLiAILab](https://github.com/FuhaiLiAiLab) where the dynamics have to respect known physical and biological structure. Memorization is one observable; the framework is the point.

---

## Walkthrough by video

The playlist — **"Quantification and decomposition of LLM and Agents — research"** — walks the same logic at whiteboard pace. Each video is one slice of the decomposition, and below I annotate each with the dynamical-systems reading, not just the memorization one.
